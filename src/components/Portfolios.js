...
const Portfolio = ({ data, onShow }) => { 
    return <MDBCol sm="12" md="4" lg="4" className="text-center">
        <span className="portfolio-btn cursor-pointer" onClick={()=>onShow(data)}>
            <img src={data.img.list[0]} alt={data.title} className={`portfolio-img ${data.isMobile ? "mobile" : ""}`} />
...
const Portfolios = ({ data, onShow }) =>
    <MDBContainer fluid className="p-5">
        <MDBRow>
            {data.map((e, i) => <Portfolio data={e} key={i} onShow={onShow} />)}
        </MDBRow>
    </MDBContainer>
