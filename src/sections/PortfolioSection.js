    ...
    render() {
        ...
        return (
            <MDBContainer MDBContainer>
                <MDBModal size="lg" isOpen={isOpen} toggle={onShow} centered className="bg-transparent">
                    <MDBModalHeader toggle={onShow} className="text-lighter elegant-color m-0 pb-0">
                        {title}
                    </MDBModalHeader>
                    <MDBModalBody className="elegant-color text-center">
                        ...
                        {!isVideo && <img src={active} className={`w-100 h-100 ${isMobile ? "mobile-modal" : ""}`} alt="img" />}
                        {isVideo && <div className="video-wrapper">
                            <video muted ref={ref => this.vidRef = ref} src={video} type="video/mp4" onClick={() => this.onPause()}></video>
                            {!isPlay && <MDBIcon icon="play" className="text-lighter btn-video" onClick={() => this.onPlay()} />}
                            {isPlay && <MDBIcon icon="pause" className="text-lighter btn-video" onClick={() => this.onPause()} />}
                        </div>}
                    </MDBModalBody>
                    <MDBModalFooter className="elegant-color">
                        <div className="modal-footer">
                            {
                                img && img.list.map((e, i) => <img src={e} key={i} alt={i} className="img-list" onClick={() => this.onActive(e, i === data.img.list.length - 1)} />)
                            }
                        </div>
                    </MDBModalFooter>
                </MDBModal>
